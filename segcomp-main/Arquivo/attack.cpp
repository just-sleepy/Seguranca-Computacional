...
int key_size(vector<string> data){
    ...
    cout<< YLW << "Possiveis tamanhos de chaves"<< NC <<endl;
    for(auto it: greaters){
        cout<< CYN << "Tamanho = "<<it.first<<" Ocorrencias = "<<it.second << NC <<endl;
    }
    
    int key_size = greaters[0].first;
    cout<<YLW<<"O tamanho escolhido foi: "<< GRN <<greaters[0].first<<NC<<endl;
    if(key_size <= 3) cout<<RED<<"O tamanho da chave e considerado pequeno, recomendamos que escolha outro tamanho."<<NC<<endl;
    char opt;
    cout<< YLW << "Voce deseja escolher o seu proprio tamanho de chave? (s/n)" << NC <<endl;
    cin>>opt;
    while(tolower(opt) != 's' and tolower(opt) != 'n'){
        cout<< RED << "Opcao inválida." << NC << endl;
        cout<< YLW << "Voce deseja escolher o seu proprio tamanho de chave? (s/n)" << NC <<endl;
        cin>>opt;
    }
    if(opt == 's'){
        ...
        while(tamanho < 2 or tamanho >20){
            cout<< RED << "Opcao invalida." << NC << endl;
            cout<< YLW << "Escolha um tamanho entre 2 e 20" << NC <<endl;
            cin>>tamanho;
        }
        key_size = tamanho;
